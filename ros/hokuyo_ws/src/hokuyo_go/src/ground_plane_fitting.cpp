...
GroundPlaneFit::GroundPlaneFit() {
    num_seg_ = 1; // Divide evenly the point cloud into a number of segments // TODO Write this implementation 1
    num_iter_ = 3; // Number of iterations 3
    num_lpr_ = 250; // number of points used to estimate the LPR 250

    th_seeds_ = 0.1; // Threshold for points to be considered initial seeds 1.2
    th_dist_ = 0.05; // Threshold distance from the plane 0.1

    sensor_height_ = 4; // LiDAR sensor height to ground #0.18
}
 ...
 */
model_t GroundPlaneFit::estimatePlane(const pcl::PointCloud<pcl::PointXYZ>& seed_points) {
 ...
 */
void GroundPlaneFit::extractInitialSeeds(const pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_in,
        const pcl::PointCloud<pcl::PointXYZ>::Ptr seed_points) {   
    
    // Sort on z-axis values (sortOnHeight)
    std::vector<pcl::PointXYZ> cloud_sorted((*cloud_in).points.begin(), (*cloud_in).points.end());
    ...
    std::vector<pcl::PointXYZ>::iterator it = cloud_sorted.begin();
 ...
 */
void GroundPlaneFit::mainLoop(const pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_in, 
	    pcl::PointCloud<pcl::PointXYZ>::Ptr notground_points,
