...
int main(void) {
    ...
    if (pcl::io::loadPCDFile <pcl::PointXYZ>("./pointcloud_009.pcd", *cloud_in) == -1)
    {
    ...
    notground_points->width = notground_points->points.size();
    notground_points->height = 1;
    std::cout << *notground_points << std::endl;
   ...
   // saveLas(ground_points, "ground.las");
    pcl::io::savePCDFile("./test_pcd.pcd", *notground_points, false);

    pcl::PCDWriter writer;
    //writer.writeASCII("test_gpf.pcd", *notground_points);
    return 0;
}
/*
void loadLas(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud, std::string path) {
